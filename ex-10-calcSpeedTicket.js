 ...
 *escribe la funcion que calcula el exceso de velocidad

 * The function should take 3 arguments:
 la funcion debe tener 3 argumentos
 *    (1) the speed limit
      limite de velocidad
 *    (2) the user's reported speed
      velocidad reportada por el usuario
 *    (3) a boolean value for whether the user was in
      un valor boleano para saber si el usuario se encontraba en una area protegida
      como zona escolar o zona de trabajo
...
function calcSpeedTicket( speedLimit, userSpeed, protectedArea){
   multaCalculada = 0;
  velocidadDeReferencia = 20;

    if(userSpeed <= speedLimit) {
       return multaCalculada
    }

    if(userSpeed < speedLimit + velocidadDeReferencia){
        multaCalculada = 150
    } else {
      multaCalculada = 250
    }
     ...
     if(protectedArea === true){
       multaCalculada = multaCalculada * 2
     }
      return multaCalculada
}
